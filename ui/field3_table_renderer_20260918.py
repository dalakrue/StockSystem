...
def render_field3_table(
    ...
    *,
    ...
    priority = [c for c in ("SL", "Strategy Decision", "Weighted Risk Decision") if c in frame.columns]
    frame = frame.loc[:, priority + [c for c in frame.columns if c not in priority]]
    if "Strategy Decision" in frame.columns and "Avg Regime Candle Rank" in frame.columns:
        cols = [c for c in frame.columns if c != "Strategy Decision"]
        cols.insert(cols.index("Avg Regime Candle Rank"), "Strategy Decision")
        frame = frame[cols]

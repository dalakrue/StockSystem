...
def _twelve_cache_file(symbol, interval, bars):
    key = hashlib.sha256(f"{symbol}|{interval}|{bars}".encode()).hexdigest()[:24]
...
def fetch_twelve(symbol="XAUUSD", api_key="", interval="1min", bars=500):
    ...
    try:
        cache_file = _twelve_cache_file(symbol, interval, bars)
        if cache_file.exists() and (time.time() - cache_file.stat().st_mtime) < TWELVE_CACHE_TTL_SECONDS:
            try:

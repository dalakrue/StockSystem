...
TWELVE_CACHE_DIR = Path("data/twelve_ohlc_cache")
TWELVE_CACHE_TTL_SECONDS = 45

_TWELVE_HTTP_LOCAL = threading.local()
_TWELVE_FETCH_LOCKS = tuple(threading.Lock() for _ in range(32))


def _twelve_http_session():
    session=getattr(_TWELVE_HTTP_LOCAL,"session",None)
    if session is None:
        session=requests.Session()
        session.mount("https://",requests.adapters.HTTPAdapter(pool_connections=4,pool_maxsize=4,max_retries=0))
        _TWELVE_HTTP_LOCAL.session=session
    return session


def _twelve_cache_file(symbol, interval, bars, api_key=""):
    # Scope cache by account, canonical symbol, interval, and capped row count.
    fingerprint=hashlib.sha256(str(api_key).encode()).hexdigest()
    identity=f"v2|{_twelve_symbol(symbol)}|{interval}|{min(max(1,int(bars)),5000)}|{fingerprint}"
    key=hashlib.sha256(identity.encode()).hexdigest()[:32]
    TWELVE_CACHE_DIR.mkdir(parents=True,exist_ok=True)
    return TWELVE_CACHE_DIR/f"{key}.pkl"


def peek_twelve_cache(symbol,api_key,interval,bars):
    if not str(api_key or "").strip():
        return None
    try:
        path=_twelve_cache_file(symbol,interval,bars,api_key)
        if not path.exists() or time.time()-path.stat().st_mtime>=TWELVE_CACHE_TTL_SECONDS:
            return None
        cached=pd.read_pickle(path)
        if isinstance(cached,pd.DataFrame) and not cached.empty:
            cached.attrs["twelve_cache_hit"]=True
            return cached
    except Exception:
        pass
    return None
...
def fetch_twelve(symbol="XAUUSD",api_key="",interval="1min",bars=500):
    # Coalesce identical in-flight fetches without sharing a requests.Session
    # between threads. Different symbols still load concurrently.
    identity=f"{_twelve_symbol(symbol)}|{interval}|{bars}|{api_key}"
    lock=_TWELVE_FETCH_LOCKS[int(hashlib.sha256(identity.encode()).hexdigest()[:8],16)%len(_TWELVE_FETCH_LOCKS)]
    with lock:
        return _fetch_twelve_once(symbol,api_key,interval,bars)


def _fetch_twelve_once(symbol="XAUUSD", api_key="", interval="1min", bars=500):
    ...
    try:
        ...
        cached=peek_twelve_cache(symbol,key,interval,bars)
        if cached is not None:
            return cached,True,"Twelve Data local OHLC cache"
        cache_file=_twelve_cache_file(symbol,interval,bars,key)
        ...
        response = _twelve_http_session().get(
        ...
        # Preserve provenance and canonical OHLC names all the way into Field 3
        # so strategy columns are calculated from the actual Twelve Data feed.
        df["data_source"] = "TWELVE_DATA"
        df["provider_symbol"] = sym
        df["source_interval"] = str(interval or "1min")

        df.attrs["twelve_cache_hit"]=False

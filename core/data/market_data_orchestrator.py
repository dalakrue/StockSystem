...
MIDDLE_STANDARD_MIN_SOURCE_BARS = 220
MIDDLE_STANDARD_MAX_SOURCE_BARS = 5000
...
class MarketDataOrchestrator:
    ...
    def provider_symbol(symbol: Any, provider: str) -> str:
        ...
        aliases = {
            ...
            ("DXY", TWELVE_FALLBACK_PROVIDER): "DXY",
            ("DXY", FCS_PROVIDER): "DXY",
        }
        ...
        for provider in provider_order[:-1]:
            ...
            for provider_attempt in range(max_attempts):
                ...
                try:
                    ...
                    attempt.update({
                        "ok": True, "request_sent": bool(metadata.get("request_sent", True)),
    ...
    def _fetch_twelve(self, *, symbol: str, provider_symbol: str, timeframe: str, bars: int, state: Mapping[str, Any]) -> Any:
        from core.connectors.data_parts.fetchers import fetch_twelve, peek_twelve_cache
        from core.twelve_data_key_pool import TwelveDataKeyPool
        ...
        # S93-S110 use EMA100/48-bar/fibonacci/oscillator structures.
        # Always load enough causal warm-up, but avoid the former 200k+ default.
        base_rows = max(max(2, int(bars)) + 24, MIDDLE_STANDARD_MIN_SOURCE_BARS)
        source_rows = min(base_rows * (4 if timeframe=="H4" else 1), MIDDLE_STANDARD_MAX_SOURCE_BARS)
        raw=None; lease=None; metadata={}
        for key_info in pool.keys():
            if assigned_alias in {"TWELVE_KEY_1","TWELVE_KEY_2"} and key_info["alias"]!=assigned_alias:
                continue
            cached=peek_twelve_cache(provider_symbol,key_info["api_key"],interval,source_rows)
            if cached is not None:
                from types import SimpleNamespace
                lease=SimpleNamespace(alias=key_info["alias"])
                raw=(cached,True,"Twelve Data local OHLC cache")
                metadata={"provider_key_alias":lease.alias,"actual_key_name":lease.alias,"request_sent":False,"cache_used":True}
                break
        if raw is None:
            ...
            if lease is None:
                ...
                if assigned_alias in {"TWELVE_KEY_1","TWELVE_KEY_2"}:
                    if snapshot.get(assigned_alias,{}).get("configured"):
                        raise ProviderRateLimited(f"{assigned_alias}_QUOTA_COOLDOWN")
                    raise ProviderPermanentError(f"{assigned_alias}_NOT_CONFIGURED")
                if any(info.get("configured") for info in snapshot.values()):
                    raise ProviderRateLimited("ALL_TWELVE_KEYS_LIMITED")
                raise ProviderPermanentError("TWELVE_DATA_KEY_POOL_NOT_CONFIGURED")
            metadata={"provider_key_alias":lease.alias,"actual_key_name":lease.alias,"masked_key":lease.masked_key,
                      "remaining_before":lease.remaining_before,"remaining_after":lease.remaining_after,
                      "selector_owned":bool(assigned_alias),"request_sent":True}
            raw=fetch_twelve(provider_symbol,lease.api_key,interval=interval,bars=source_rows)

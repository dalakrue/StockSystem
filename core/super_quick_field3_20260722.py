...
FAST_RUN_VERSION = "field3-three-table-fast-v4-20260729"
...
def _persist_middle_finder_history(state: MutableMapping[str, Any], frames: Mapping[str, pd.DataFrame], *, timeframe: str, signature: str) -> dict[str, Any]:
    ...
    generated = _build_middle_finder_history(frames, timeframe=timeframe)
    if generated.empty:
        return {"ok": False, "reused": False, "rows": 0, "error": "NO_HISTORICAL_ROWS_FROM_LOADED_FRAMES"}
    frames_to_merge = [generated]
    try:
        if MIDDLE_HISTORY_FILE.exists():
            saved = pd.read_parquet(MIDDLE_HISTORY_FILE)
            if isinstance(saved, pd.DataFrame) and not saved.empty:
                frames_to_merge.insert(0, saved)
    ...
    try:
        if MIDDLE_HISTORY_CSV_FILE.exists():
            ...
            if isinstance(saved_csv, pd.DataFrame) and not saved_csv.empty:
                frames_to_merge.insert(0, saved_csv)
    except Exception as exc:
        state["field3_middle_history_backfill_csv_load_warning_20260818"] = f"{type(exc).__name__}: {exc}"
    current = state.get(MIDDLE_HISTORY_STATE_KEY)
    if isinstance(current, pd.DataFrame) and not current.empty:
        frames_to_merge.append(current)
    merged = pd.concat(frames_to_merge, ignore_index=True, sort=False)
    merged["Datetime"] = pd.to_datetime(merged["Datetime"], errors="coerce")
    try:
        if getattr(merged["Datetime"].dt, "tz", None) is not None:
    ...
    try:
        MIDDLE_HISTORY_CSV_FILE.parent.mkdir(parents=True, exist_ok=True)
        merged.to_csv(MIDDLE_HISTORY_CSV_FILE, index=False, encoding="utf-8")
    except Exception as exc:
        state["field3_middle_history_backfill_csv_save_warning_20260818"] = f"{type(exc).__name__}: {exc}"
...
def _fast_standard(frame: pd.DataFrame, *, standard: str, window_bars: int) -> dict[str, Any]:
    ...
    volatility so a 30-symbol loaded universe can be processed in seconds rather
    ...
    for index, symbol in enumerate(symbols, start=1):
        try:
            ...
            decision_strength = float(
                0.60 * higher["reliability"]
                + 0.25 * middle["reliability"]
                + 0.15 * lower["reliability"]
            )
            rank_payload = {
                "fast_lane_version": FAST_RUN_VERSION,
                "higher": higher,
            ...
            ranking_rows.append({
                ...
                "Lower Probability": lower["probability"], "Lower Reliability": lower["reliability"],
...
def run_super_quick_field3(
    ...
    result = {
        ...
        "provider_requests": 0,
        "deferred_to_quick": [
            "Lower Standard Summary table", "Middle Standard Summary table", "Final Cross-Symbol Ranking",

...
def render_responsive_records(
    ...
    *,
    ...
    force_table_on_phone: bool = False,
    ...
    # Canonical visibility order: execution-risk information must be visible
    # immediately.  Every Field table using this renderer receives the same
    # priority order instead of relying on individual tab column lists.
    priority_columns = [
        "SL",
        "Strategy Decision",
        "Weighted Risk Decision",
    ]
    existing_priority = [c for c in priority_columns if c in frame.columns]
    remaining_columns = [c for c in frame.columns if c not in existing_priority]
    frame = frame.loc[:, existing_priority + remaining_columns]
    ...
    def _style_middle_extremes(
        ...
        return styled
    if (not is_phone_mode(state)) or force_table_on_phone:

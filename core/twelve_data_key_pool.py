...
def resolve_twelve_key(state: Mapping[str, Any] | None, alias: str) -> str:
    """Resolve any configured Twelve Data pool key (1..4)."""
    state_map = state if _state_getter_compatible(state) else {}
    text = str(alias or "TWELVE_KEY_1").upper().replace("API_KEY", "KEY").replace("DATA_", "")
    import re
    m = re.search(r"(?:KEY|TWELVE)(?:_?)([1-4])$", text)
    index = int(m.group(1)) if m else 1
    # Key 1 also accepts the legacy single-key names.
    names = (f"twelve_api_key_{index}", f"twelve_data_api_key_{index}", f"TWELVE_DATA_API_KEY_{index}", f"TWELVE_API_KEY_{index}")
    if index == 1:
        names += ("twelve_api_key", "second_api_key", "TWELVE_DATA_API_KEY", "TWELVE_API_KEY", "twelve_data_api_key")
    secret_paths = [("twelve_data", f"api_key_{index}"), ("twelve_data", f"key_{index}"), ("twelve", f"api_key_{index}"), ("twelve", f"key_{index}"), ("api_keys", f"twelve_data_key_{index}"), ("api_keys", f"twelve_api_key_{index}")]
    if index == 1:
        secret_paths += [("api_keys", "second_api"), ("api_keys", "twelve_data"), ("api_keys", "twelve"), ("twelve_data", "api_key"), ("twelve", "api_key")]
    env_names = (f"TWELVE_DATA_API_KEY_{index}", f"TWELVE_API_KEY_{index}")
    vault_names = (f"TWELVE_DATA_KEY_{index}", f"TWELVE_KEY_{index}", "TWELVE_DATA_API_KEY")
    return _first_value(state_map, names, tuple(secret_paths), env_names, vault_names)
...
class TwelveDataKeyPool:
    def __init__(self, state: MutableMapping[str, Any] | Mapping[str, Any] | None = None, *, minute_limit: int | None = None) -> None:
        ...
        self.rotation_cursor = int(self.read_state.get("twelve_data_rotation_cursor", 0) or 0)
    ...
    def keys(self) -> list[dict[str, Any]]:
        ...
        for alias in ("TWELVE_KEY_1", "TWELVE_KEY_2", "TWELVE_KEY_3", "TWELVE_KEY_4"):
    ...
    def reserve_key(self, *, symbol: Any = "", timeframe: Any = "") -> TwelveDataKeyLease | None:
        with _LOCK:
            ...
            if not candidates:
                return None
            # True round-robin across available keys; quotas/cooldowns still gate selection.
            candidates.sort(key=lambda value: value[2]["alias"])
            preferred = self.rotation_cursor % len(candidates)
            remaining, _, key_info, item = candidates[preferred]
            self.rotation_cursor = (preferred + 1) % len(candidates)
            try:
                self.state["twelve_data_rotation_cursor"] = self.rotation_cursor
            except Exception:
                pass
    ...
    def reserve_alias(self, alias: str, *, symbol: Any = "", timeframe: Any = "") -> TwelveDataKeyLease | None:
        ...
        if normalized_alias in {"KEY_3", "TWELVE_DATA_KEY_3", "TWELVE_API_KEY_3"}:
            normalized_alias = "TWELVE_KEY_3"
        if normalized_alias in {"KEY_4", "TWELVE_DATA_KEY_4", "TWELVE_API_KEY_4"}:
            normalized_alias = "TWELVE_KEY_4"
        with _LOCK:

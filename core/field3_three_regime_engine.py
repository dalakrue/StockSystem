...
def _fit_standard(frame: pd.DataFrame, timeframe: str, standard: str, window_bars: int) -> StandardEvidence:
    ...
    # Price at the first candle of the current standard regime. ``states`` is
    # aligned to ``sample`` rows after the initial return, so age=1 is the
    # latest candle and age=2 is the preceding candle. Prefer the candle open
    # (the unambiguous starting price), with close as a safe fallback.
    regime_start_index = max(0, len(sample) - int(age))
    price_series = sample.get("open")
    if price_series is None:
        price_series = sample.get("close")
    try:
        regime_start_price = float(pd.to_numeric(price_series, errors="coerce").iloc[regime_start_index])
        if not np.isfinite(regime_start_price):
            regime_start_price = None
    except Exception:
        regime_start_price = None
    try:
        regime_start_time = str(sample["open_time"].iloc[regime_start_index].isoformat())
    except Exception:
        regime_start_time = ""
    ...
    payload = {
        ...
        "regime_start_price": regime_start_price,
        "regime_start_time": regime_start_time,
        "average_regime_candle_count_before_change": completed_run_stats.get("average"),
        "completed_regime_run_count": int(completed_run_stats.get("count") or 0),
        "average_regime_duration_basis": completed_run_stats.get("basis"),
        "change_probability_history": cp_history[-60:],
        "feature_evidence": {
    ...
    *,
    ...
    for symbol, frame in standardized.items():
        ...
        row = {
            ...
            "Starting Price": evidence["MIDDLE"].payload.get("regime_start_price"),
            "Middle Avg Regime Candle Count Before Regime Change": evidence["MIDDLE"].payload.get("average_regime_candle_count_before_change"),
            "Middle Completed Regime Sample Count": int(evidence["MIDDLE"].payload.get("completed_regime_run_count") or 0),

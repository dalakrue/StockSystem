...
from core.auth_reconnect_session_20260818 import (
    issue_session as _issue_reconnect_session,
    restore_session as _restore_reconnect_session,
    revoke_session as _revoke_reconnect_session,
)

UNIQUE = "20260612_galileo_auth_v2"
AUTH_TOKEN_QUERY_KEY = "new7_session"
AUTH_TARGET_QUERY_KEY = "new7_target"
...
# -----------------------------------------------------------------------------
# Persistent browser reconnect lease
# -----------------------------------------------------------------------------
...
def _set_query_value(key: str, value: str) -> None:
    clean = str(value or "").strip()
    try:
        if clean:
            st.query_params[key] = clean
...
def _apply_authenticated_state(email: str, *, guest: bool, route_to_settings: bool) -> None:
    ...
    if not route_to_settings:
        return
    # Every explicit fresh login starts in Settings. A reconnect never resets
    # navigation; the one-time target query controls post-run Field 3 recovery.
    st.session_state["active_page"] = "Settings"
    st.session_state["tab_choice"] = "Settings"
    st.session_state["requested_page"] = "Settings"
    st.session_state["requested_subpage"] = ""
    st.session_state["first_settings_route_committed_20260706"] = False
    st.session_state.pop("guest_lunch_route_committed_20260704", None)


def _issue_browser_reconnect_token(email: str, *, guest: bool) -> str:
    token = _issue_reconnect_session(_db_path(), email=email, guest=guest)
    _set_query_value(AUTH_TOKEN_QUERY_KEY, token)
    st.session_state["new7_auth_reconnect_lease_ready_20260818"] = True
    return token


def clear_persistent_auth_session() -> None:
    token = _query_value(AUTH_TOKEN_QUERY_KEY)
    if token:
        try:
            _revoke_reconnect_session(_db_path(), token)
        except Exception:
            pass
    _set_query_value(AUTH_TOKEN_QUERY_KEY, "")
    _set_query_value(AUTH_TARGET_QUERY_KEY, "")
    st.session_state.pop("new7_auth_reconnect_lease_ready_20260818", None)


def request_persistent_auth_route(target: str) -> None:
    """Persist one safe post-run route across a browser/WebSocket reconnect."""
    normalized = "field3" if str(target or "").strip().lower().replace(" ", "") == "field3" else "settings"
    _set_query_value(AUTH_TARGET_QUERY_KEY, normalized)


def _apply_persistent_auth_route() -> None:
    target = _query_value(AUTH_TARGET_QUERY_KEY).strip().lower()
    if target not in {"field3", "settings"}:
        return
    page = "Field 3" if target == "field3" else "Settings"
    st.session_state["active_page"] = page
    st.session_state["tab_choice"] = page
    st.session_state["requested_page"] = page
    st.session_state["requested_subpage"] = ""
    if page == "Field 3":
        st.session_state["active_field"] = "Field 3"
        st.session_state["field_3_expanded"] = True
        st.session_state["scroll_target"] = "field-3-anchor"
        st.session_state["lunch_calculation_completed_notice_20260621"] = True
    st.session_state["auth_reconnect_route_applied_20260818"] = page
    _set_query_value(AUTH_TARGET_QUERY_KEY, "")
...
def _login_success(email: str, guest: bool = False) -> None:
    ...
    clean = str(email or "Guest").strip() or "Guest"
    _apply_authenticated_state(clean, guest=guest, route_to_settings=True)
    try:
        _issue_browser_reconnect_token(clean, guest=guest)
    except Exception as exc:
        # Login remains usable even if the optional reconnect lease cannot be
        # written. The error is visible in diagnostics without exposing tokens.
        st.session_state["new7_auth_reconnect_issue_error_20260818"] = f"{type(exc).__name__}: {exc}"
...
def _brand_panel() -> None:
    st.markdown(
        ...
        <div class="new7-brand-panel"><div class="new7-brand-inner">
            ...
            <div class="new7-brand-title">Top-30 Multi-Symbol<br/>Decision App</div>
            <div class="new7-brand-sub">Three-selector market loading, fast all-loaded regime ranking, PowerBI-style projection, research NLP, data mining priority and mobile-first copy output for your final-year CS project.</div>
            <div class="new7-feature-mini">
                <div>📊 Projection + actual/error</div>
                <div>🧠 Data Analysis / Mining / NLP</div>
                <div>⚡ All-loaded Super Quick</div>
...
def render_auth_gate() -> bool:
    ...
    if not st.session_state.get("new7_auth_logged_in"):
        reconnect_token = _query_value(AUTH_TOKEN_QUERY_KEY)
        if reconnect_token:
            try:
                restored = _restore_reconnect_session(_db_path(), reconnect_token)
            except Exception as exc:
                restored = None
                st.session_state["new7_auth_reconnect_restore_error_20260818"] = f"{type(exc).__name__}: {exc}"
            if isinstance(restored, dict):
                _apply_authenticated_state(
                    str(restored.get("email") or "Guest"),
                    guest=bool(restored.get("guest")),
                    route_to_settings=False,
                )
                st.session_state["new7_auth_reconnected_20260818"] = True
            else:
                _set_query_value(AUTH_TOKEN_QUERY_KEY, "")
                _set_query_value(AUTH_TARGET_QUERY_KEY, "")

    if st.session_state.get("new7_auth_logged_in"):
        if not _query_value(AUTH_TOKEN_QUERY_KEY):
            try:
                _issue_browser_reconnect_token(
                    str(st.session_state.get("new7_auth_email") or "Guest"),
                    guest=bool(st.session_state.get("new7_auth_guest")),
                )
            except Exception as exc:
                st.session_state["new7_auth_reconnect_issue_error_20260818"] = f"{type(exc).__name__}: {exc}"
        _apply_persistent_auth_route()

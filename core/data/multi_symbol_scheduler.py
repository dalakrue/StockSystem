...
class MultiSymbolScheduler:
    ...
    def run(
        ...
        *,
        ...
        safe_parallel_workers=1
        selector_assigned_alias=str(state_map.get("selector_owned_twelve_assigned_key_20260708") or "").strip().upper()
        if primary_provider=="TWELVE_DATA_KEY_POOL":
            try:
                from core.twelve_data_key_pool import TwelveDataKeyPool
                pool=TwelveDataKeyPool.from_state(state)
                allowances=pool.status_snapshot()
                if selector_assigned_alias in {"TWELVE_KEY_1","TWELVE_KEY_2"}:
                    allowances={selector_assigned_alias:allowances.get(selector_assigned_alias,{})}
                available=sum(int(v.get("remaining_credits",0)) for v in allowances.values() if v.get("configured") and not v.get("cooldown_active"))
                if likely_live_count<=available:
                    safe_parallel_workers=min(4,max(1,len(plan)))
            except Exception:
                safe_parallel_workers=1

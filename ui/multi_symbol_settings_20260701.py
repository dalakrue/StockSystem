...
30-symbol ranking universe.  The chosen calculation button changes depth, not
...
SUPPORTED_SYMBOLS: tuple[str, ...] = TARGET_FX_SYMBOLS
...
FIRST_BEST_10_CURRENCY_PAIRS: list[str] = list(FIRST_TARGET_10)
SECOND_BEST_10_CURRENCY_POOL: list[str] = list(SECOND_TARGET_10)
THIRD_BEST_10_SPREAD_UNDER_25: list[str] = []
...
PRESET_GROUPS_20260702: dict[str, list[str]] = {
    "Fixed FX 1–10": list(FIRST_TARGET_10),
    "Fixed FX 11–20": list(SECOND_TARGET_10),
}



def normalize_symbol(value: Any, default: str = "AUDCAD") -> str:
    raw = str(value or default).strip().upper().replace("/", "").replace("_", "").replace(" ", "")
...
def _render_group_card(state: MutableMapping[str, Any], group_name: str) -> list[str]:
    ...
    item = definitions[group_name]
    fixed = (
        list(FIRST_TARGET_10) if group_name == "FIRST"
        else list(SECOND_TARGET_10) if group_name == "SECOND"
        else []
    )
    state[item["state_key"]] = list(fixed)
    # Widget state is seeded before instantiation and never modified afterwards.
    state[item["widget_key"]] = list(fixed)
    with st.container(border=True):
        st.markdown(f"### 🌐 {item['title']}")
        if group_name == "THIRD":
            st.caption("Disabled. The app is locked to the requested 20 FX symbols only.")
        else:
            st.caption("Fixed symbol group. Only the requested 20 FX pairs can be loaded.")
        selected_value = st.multiselect(
            f"Fixed symbols for {item['title']}",
            options=list(SUPPORTED_SYMBOLS),
            key=item["widget_key"],
            max_selections=int(item["limit"]),
            disabled=True,
        )
        selected_value = normalize_selected(selected_value, limit=int(item["limit"]))
        ...
        metrics[1].metric("Group Limit", int(item["limit"]))
        metrics[2].metric("Timeframe", timeframe)
        if selected_value:
            st.caption("Fixed order: " + " → ".join(selected_value))
    return selected_value


def render_multi_symbol_selectors(
    state: MutableMapping[str, Any] | None = None,
    *,
    foreground_container: Any | None = None,
    load_progress_container: Any | None = None,
) -> dict[str, list[str]]:
    """Render three selectors and merge up to 30 unique symbols.

    ``foreground_container`` is an optional early layout slot.  Settings
    creates it immediately below the top Super Quick action, then this function
    fills it after the selector state is prepared.  Streamlit therefore keeps
    the complete Foreground Symbol Loading Control Center at the requested
    visual position without duplicating its widgets or load transaction.
    """
    ...
    st.caption("The app is locked to one canonical 20-symbol FX universe. XAUUSD and all other symbols are excluded at the loader boundary.")
    ...
    try:
        from core.current_result_sync_20260708 import sync_settings_source_of_truth
        sync_settings_source_of_truth(state, configured_union[:20] or ["AUDCAD"], state.get("timeframe") or state.get("selected_timeframe") or "H4", reason="selectors_render_current_source_of_truth")
    ...
    # The loading board can be painted into a placeholder created above the
    # Settings controls.  ``nullcontext`` keeps older callers unchanged.
    foreground_target = (
        foreground_container.container()
        if foreground_container is not None and hasattr(foreground_container, "container")
        else nullcontext()
    )
    with foreground_target, st.container(border=True):
        timeframe = str(state.get("timeframe") or state.get("selected_timeframe") or "H4").upper()
        try:
        ...
        st.markdown("### Foreground Loading Workers")
        st.caption("Twelve Key 1 Worker")
        st.caption("Assigned selector: Selector 1")
        st.caption("Twelve Key 2 Worker")
        st.caption("Assigned selector: Selector 2")
        st.caption("Shared Key-Pool Worker")
        st.caption("Assigned selector: Selector 3")
        st.caption("Top action: Load All Selected")

        st.markdown("### Foreground Symbol Loading Control Center")
        # Persistent progress location. The actual progress widget is created
        # when Load All Selected starts; this anchor keeps the progress area in
        # the same page and prevents it appearing below unrelated controls.
        if state.get("foreground_load_progress_text_20260824"):
            st.info(state.get("foreground_load_progress_text_20260824"))
        ...
        st.caption("Top action: Load All Selected runs the same three-worker pipeline below and shows live progress in this page.")
        st.markdown("#### Canonical Top-30 Universe Preview")
        ...
        if len(configured_union) > len(canonical_selected):
            ...
            st.warning("Only the first 30 deduplicated symbols are used. Skipped: " + " → ".join(skipped))
        ...
        # The all-selected action is intentionally rendered at the top of the
        # Settings sidebar.  Keep this slot as a compact status marker so the
        # legacy five-column layout remains stable without showing a duplicate
        # button lower in the page.
        load_all_clicked = bool(state.pop("settings_top_load_all_requested_20260823", False)) and bool(canonical_selected)
        button_cols_20260708[3].caption("Top action: Load All Selected")
        ...
        # One-time automatic Super Quick trigger after the app opens when a validated
        # load already exists. This removes the need to manually click the button.
        auto_super_quick_start = bool(state.get("auto_super_quick_after_open_20260917", False))
        if not auto_super_quick_start and not load_s1_clicked and not load_s2_clicked and not load_s3_clicked and not load_all_clicked:
            if state.get("canonical_loaded_symbols_20260701") or state.get("completed_symbols"):
                state["auto_super_quick_after_open_20260917"] = True
        ...
        if action_requested:
            ...
            st.info("Loading progress is live: Twelve Key 1 Worker + Twelve Key 2 Worker + Shared Key-Pool Worker are running through the canonical top-30 universe.")
            progress_bar = st.progress(0.0, text=action_label)
            # Keep a second, compact progress bar at the top Quick Actions
            # row.  It is intentionally allowed to duplicate the detailed
            # loader bar so mobile users never need to scroll for completion.
            top_progress_bar = load_progress_container
            ...
            def _load_progress(snapshot: Any) -> None:
                ...
                if top_progress_bar is not None and hasattr(top_progress_bar, "progress"):
                    top_progress_bar.progress(min(1.0, max(0.0, percent / 100.0)), text=f"{percent:.1f}% loaded · {completed}/{total} symbols")
            ...
            try:
                ...
                if top_progress_bar is not None and hasattr(top_progress_bar, "progress"):
                    top_progress_bar.progress(1.0, text="100.0% loaded · Selector-owned load transaction completed")
                if failed_count:
                    st.warning(
                ...
                # The top load action is intentionally an all-in-one workflow:
                # once validated frames exist, publish the fast three-table
                # Field 3 result before rerendering. Finder history is never
                # built here; it has its own explicit Run Finder action.
                try:
                    from core.super_quick_field3_20260722 import run_super_quick_field3
                    loaded_for_super = normalize_selected(
                        final_status.get("loaded_symbols") or canonical_selected,
                        limit=None,
                    )
                    if loaded_for_super:
                        ...
                        state["startup_top30_load_status_20260824"] = "LOADED_CALCULATING"

                        def _load_then_calculate_progress(snapshot: Any) -> None:
                            if not isinstance(snapshot, dict):
                                return
                            pct = float(snapshot.get("overall_percent") or 0.0)
                            stage = str(snapshot.get("current_stage") or "Calculating Field 3")
                            if top_progress_bar is not None and hasattr(top_progress_bar, "progress"):
                                top_progress_bar.progress(
                                    min(1.0, max(0.0, pct / 100.0)),
                                    text=f"{pct:.1f}% — {stage}",
                                )

                        auto_result = run_super_quick_field3(
                            state,
                            symbols=loaded_for_super,
                            timeframe=timeframe,
                            progress_callback=_load_then_calculate_progress,
                        )
                        state["settings_run_status_20260617"] = auto_result
                        state["startup_top30_load_status_20260824"] = (
                            "READY_WITH_SUPER_QUICK" if auto_result.get("ok") else "LOADED_SUPER_QUICK_FAILED"
                        )
                        st.success(
                            f"Top-30 load + Super Quick completed for {int(auto_result.get('completed_symbols') or 0)} symbol(s)."
                        )
                except Exception as auto_calc_exc:
                    state["startup_top30_load_status_20260824"] = "LOADED_CALCULATION_FAILED"
                    state["startup_top30_super_quick_error_20260824"] = f"{type(auto_calc_exc).__name__}: {auto_calc_exc}"
                    st.warning(
                        "Top-30 data loaded, but Super Quick was not published: "
                        f"{type(auto_calc_exc).__name__}: {auto_calc_exc}"
                    )
                st.caption(
                    "Provider summary: "
                ...
                progress_bar.empty()
                if top_progress_bar is not None and hasattr(top_progress_bar, "error"):
                    top_progress_bar.error(f"Load stopped safely: {type(load_exc).__name__}: {load_exc}")
                progress_text.empty()

...
from core.fixed_fx_universe_20260918 import TARGET_FX_SET, TARGET_FX_SYMBOLS, filter_target_symbols

FAST_RUN_VERSION = "field3-fixed20-fast-v6-strategy-scores-20260918"
...
def _historical_fast_standard(frame: pd.DataFrame, *, window_bars: int) -> pd.DataFrame:
    ...
    window = max(8, min(int(window_bars), max(8, len(df) - 1)))
...
def _build_middle_finder_history(frames: Mapping[str, pd.DataFrame], *, timeframe: str) -> pd.DataFrame:
    ...
    for symbol, frame in frames.items():
        ...
        source = frame.tail(n).reset_index(drop=True).copy()
        ...
        block = block.loc[block["Middle Standard Regime State"].ne("UNAVAILABLE")].copy()
    ...
    preferred = [
        "Strategy Decision", "Avg Regime Candle Rank", "Symbol", "Lower Regime Bias", "Middle Regime Bias",
...
def _persist_middle_finder_history(state: MutableMapping[str, Any], frames: Mapping[str, pd.DataFrame], *, timeframe: str, signature: str) -> dict[str, Any]:
    """Persist Finder history once, without duplicate parquet+CSV+session merges."""
    fixed_frames = {symbol: frame for symbol, frame in frames.items() if str(symbol).upper() in TARGET_FX_SET}
    if str(state.get(MIDDLE_HISTORY_BACKFILL_SIGNATURE_KEY) or "") == str(signature):
        existing = state.get(MIDDLE_HISTORY_STATE_KEY)
        return {"ok": True, "reused": True, "rows": int(len(existing)) if isinstance(existing, pd.DataFrame) else 0}
    generated = _build_middle_finder_history(fixed_frames, timeframe=timeframe)
    if generated.empty:
        return {"ok": False, "reused": False, "rows": 0, "error": "NO_HISTORICAL_ROWS_FROM_LOADED_FRAMES"}

    # Prefer the in-session history. If absent, read parquet; CSV is fallback
    # only. Reading all three copies was a major Finder startup cost.
    existing = state.get(MIDDLE_HISTORY_STATE_KEY)
    if not isinstance(existing, pd.DataFrame) or existing.empty:
        existing = pd.DataFrame()
        try:
            if MIDDLE_HISTORY_FILE.exists():
                existing = pd.read_parquet(MIDDLE_HISTORY_FILE)
        except Exception as exc:
            state["field3_middle_history_backfill_load_warning_20260818"] = f"{type(exc).__name__}: {exc}"
        if existing.empty:
            try:
                if MIDDLE_HISTORY_CSV_FILE.exists():
                    existing = pd.read_csv(MIDDLE_HISTORY_CSV_FILE, low_memory=False)
            except Exception as exc:
                state["field3_middle_history_backfill_csv_load_warning_20260818"] = f"{type(exc).__name__}: {exc}"

    merge_parts = [x for x in (existing, generated) if isinstance(x, pd.DataFrame) and not x.empty]
    merged = pd.concat(merge_parts, ignore_index=True, sort=False)
    if "Symbol" in merged.columns:
        normalized = merged["Symbol"].astype(str).str.upper().str.replace("/", "", regex=False).str.replace("_", "", regex=False).str.replace(" ", "", regex=False)
        merged = merged.loc[normalized.isin(TARGET_FX_SET)].copy()
        merged["Symbol"] = normalized.loc[merged.index]
    merged["Datetime"] = pd.to_datetime(merged["Datetime"], errors="coerce", utc=True).dt.tz_localize(None)
    merged = merged.dropna(subset=["Datetime"]).drop_duplicates(subset=["Datetime", "Symbol"], keep="last")
    merged = merged.sort_values(["Datetime", "Symbol"], kind="mergesort").reset_index(drop=True)
    state[MIDDLE_HISTORY_STATE_KEY] = merged
    state[MIDDLE_HISTORY_BACKFILL_SIGNATURE_KEY] = str(signature)
    state["field3_middle_history_backfill_rows_20260818"] = int(len(merged))
    try:
        MIDDLE_HISTORY_FILE.parent.mkdir(parents=True, exist_ok=True)
        merged.to_parquet(MIDDLE_HISTORY_FILE, index=False)
    except Exception as exc:
        state["field3_middle_history_backfill_save_warning_20260818"] = f"{type(exc).__name__}: {exc}"
    # CSV is an export fallback, not a second mandatory database write.
    return {"ok": True, "reused": False, "rows": int(len(merged)), "generated_rows": int(len(generated))}
...
def _fast_standard(frame: pd.DataFrame, *, standard: str, window_bars: int) -> dict[str, Any]:
    ...
    volatility so the fixed 20-symbol loaded universe can be processed in seconds rather
    than entering the heavy Quick/Full research pipeline.
    """
    df = standardize_candles(frame)
    ...
    completed_run_stats = completed_regime_run_stats(states.tolist(), current_value=current_state)
    bias = "BUY" if current_state == "BULL" else "SELL" if current_state == "BEAR" else "NEUTRAL"
    # Super Quick no longer calculates posterior/persistence/possibility math.
    # The compatibility fields below remain numeric only because the existing
    # persistence schema expects them; they are not used for the fast ranking.
    signal_strength = min(1.0, abs(float(score.iloc[-1])))
    probability = 0.0
    persistence = 0.0
    expected_duration = float(age)
    transition_risk = 0.0
    reliability = 0.0
    completeness = min(1.0, len(close) / max(45.0, float(window_bars)))
    quality_grade = "A" if completeness >= 0.95 else "B" if completeness >= 0.75 else "C" if completeness >= 0.50 else "D"
    try:
        starting_price = float(pd.to_numeric(df["open"], errors="coerce").iloc[max(0, len(df) - int(age))])
        if not np.isfinite(starting_price):
            starting_price = None
    except Exception:
        starting_price = None
    ...
    return {
        ...
        "avg_regime_candle_count_before_change": completed_run_stats.get("average"),
        "completed_regime_run_count": int(completed_run_stats.get("count") or 0),
        "avg_regime_duration_basis": completed_run_stats.get("basis"),
...
def _extract_loaded_frames(state: Mapping[str, Any], requested_symbols: list[str]) -> tuple[dict[str, pd.DataFrame], dict[str, str]]:
    ...
    """
    requested_symbols = filter_target_symbols(requested_symbols, limit=20)
    report = state.get(MARKET_RESULTS_KEY)
    report = report if isinstance(report, Mapping) else {}
    raw_results = report.get("results") if isinstance(report.get("results"), Mapping) else {}
    canonical_frames = state.get("canonical_symbol_candles")
    canonical_frames = canonical_frames if isinstance(canonical_frames, Mapping) else {}
    ...
    for symbol in requested_symbols:
        ...
        if frame.empty:
            frame = standardize_candles(canonical_frames.get(symbol))
        if frame.empty:
...
def _fast_input_signature(
    ...
    for symbol in selected:
        ...
        bounded = frame.loc[:, columns].tail(800)
...
def run_super_quick_field3(
    ...
    selected = filter_target_symbols(symbols or list(context.loaded_symbols), limit=20)
    tf = str(timeframe or context.timeframe or state.get("timeframe") or "H1").strip().upper() or "H1"
    if not selected:

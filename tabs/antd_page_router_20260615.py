...
def _save_and_connect_twelve_callback(widget_key: str | None = None) -> None:
    ...
    try:
        ...
        result = MarketDataOrchestrator().fetch(
            ...
            bars=max(5, int(st.session_state.get("connector_bars", 200000) or 200000)),
...
def _open_lunch_ai_after_settings_run(*, used_previous: bool = False) -> None:
    ...
    # "lunch_field_open_3_20260621": True
    for number in range(1, 12):
        updates[f"lunch_field_open_{number}_20260621"] = number == 3
        updates[f"lunch_field_widget_{number}_20260621"] = number == 3
    try:
        # If the long calculation caused a browser/WebSocket reconnect, the
        # opaque auth lease restores the signed-in session and this one-use URL
        # target still opens Field 3 instead of showing Login or Settings.
        from core.light_auth_20260612 import request_persistent_auth_route
        request_persistent_auth_route("Field 3")
    except Exception as auth_route_exc_20260818:
        st.session_state["post_run_field3_auth_route_error_20260818"] = (
            f"{type(auth_route_exc_20260818).__name__}: {auth_route_exc_20260818}"
        )
    try:
...
def _render_home() -> None:
    ...
    with st.container(border=True):
        ...
        tool_rows = pd.DataFrame([
            {"Tool": "Settings", "Use": "Choose symbols/timeframe, connect providers, run Super Quick.", "Ready": "Yes"},
...
def _render_market_connector_section(
    *,
    include_multi_symbol_controls: bool = False,
    foreground_container: Any | None = None,
    load_progress_container: Any | None = None,
):
    ...
    with st.container(border=True):
        ...
        try:
            ...
            st.session_state["connector_bars"] = int(st.number_input(
                "Candles / bars", min_value=100, max_value=1000000,
                value=int(st.session_state.get("connector_bars", 200000) or 200000), step=100,
        ...
        if include_multi_symbol_controls:
            ...
            groups = render_multi_symbol_selectors(
                st.session_state,
                foreground_container=foreground_container,
                load_progress_container=load_progress_container,
            )
...
def _render_settings() -> None:
    ...
    if not st.session_state.get("settings_h1_default_initialized_20260729"):
        # Upgrade the old automatic H4 choice to H1 once. After this one-time
        # migration, any timeframe the user selects remains authoritative.
        ...
        # Migrate the keyed Settings widgets too; otherwise an already-open
        # Streamlit session can keep the old H4 widget value even after the
        # canonical timeframe state has moved to H1.
    ...
    # Restore the persisted selector profile and start one top-30 load on the
    # first Settings render. The selector renderer consumes this request after
    # it has rehydrated all three groups and immediately follows it with the
    # compute-only Super Quick publication.
    if not st.session_state.get("startup_top30_load_attempted_20260824"):
        ...
        # Auto trigger the same Super Quick path after app opening so the user
        # does not need to scroll and click the button manually. The loader
        # still owns data readiness; this only queues the existing action.
    ...
    # High-frequency controls stay at the top of the Settings sidebar.  The
    # selector/load state is resolved below, so clicks are carried forward as a
    # one-pass request and executed by the canonical loader/calculation path.
    st.markdown("#### ⚡ Quick Actions")
    # High visibility progress area: kept directly above the action buttons so
    # loading/calculation status is visible without scrolling through Settings.
    top_progress_slot_20260823 = st.empty()
    quick_action_cols = st.columns(2)
    top_load_all_clicked = quick_action_cols[0].button(
        "📥 Load All Selected (Top 30)",
        key="settings_top_load_all_selected_20260823",
        use_container_width=True,
        help="Load the complete selected top-30 universe with the three selector workers.",
    )
    top_super_quick_clicked = quick_action_cols[1].button(
        "⚡ Super Quick Run",
        key="settings_top_super_quick_20260823",
        use_container_width=True,
        help="Run the bounded Field 3 calculation on already-loaded candles for an instant result.",
    )
    # Dedicated load progress anchor sits directly beneath the Load button;
    # the loader also updates this same anchor during the synchronous request.
    ...
    try:
        ...
        pass
    # Reserve the exact visual slot immediately below the top Super Quick
    # action.  The selector renderer fills this placeholder with the complete
    # Foreground Symbol Loading Control Center after it has prepared its state,
    # so the load board is not duplicated or detached from its widgets.
    ...
    # Render the selector and the single visible Super Quick calculation action.
    # The older Quick/Full modes remain internal compatibility scopes only.
    ...
    try:
        connector_controls = _render_market_connector_section(
            include_multi_symbol_controls=True,
            foreground_container=foreground_loading_slot_20260823,
            load_progress_container=top_load_progress_slot_20260824,
        )
        ...
        symbol_groups_20260706 = render_multi_symbol_selectors(
            st.session_state,
            foreground_container=foreground_loading_slot_20260823,
            load_progress_container=top_load_progress_slot_20260824,
        )
    ...
    try:
        ...
        try:
            ...
            sync_settings_source_of_truth(
                ...
                st.session_state.get("timeframe") or "H1",
    ...
    for slot, title, status in (
        (load_summary_cols[0], "Selector 1 Load", first_load_status_20260707),
        (load_summary_cols[1], "Selector 2 Load", second_load_status_20260707),
        (load_summary_cols[2], "Selector 3 Load", third_load_status_20260707),
    ):
    ...
    load_summary_cols[3].metric(
        "All Run Buttons",
        f"{cumulative_ready_count_20260727} READY",
        delta=(f"{cumulative_failed_count_20260727} not ready" if cumulative_failed_count_20260727 else "ALL LOADED"),
    )
    st.caption(
        "The three left cards are load diagnostics only. Super Quick, Quick, and Full all calculate the same "
        f"{cumulative_ready_count_20260727}-symbol loaded universe; the buttons differ only in calculation depth."
    )
    ...
    # Keep one deliberately bounded run action in the UI.  The old Full button
    # exposed the slow thesis/research pipeline and is intentionally not shown.
    # Super Quick uses the existing compute-only Field 3 fast lane and returns
    # the compact result from the loaded candles without another API fetch.
    run_disabled_20260707 = not bool(cumulative_load_status_20260707.get("loaded_symbols"))
    # The live Super Quick control is rendered at the top of the sidebar.  Its
    # request is consumed here after the cumulative loaded-universe check.
    ...
    if run_clicked_20260701:
        ...
        # Super Quick fast path: use the direct cached Field 3 engine instead of
        # the generic instant queue. This restores the old one-click behaviour:
        # loaded candles -> cached calculation -> immediate publication.
        # It does not affect Quick/Full paths.
        if selected_scope_20260701 == "LUNCH_CORE":
            try:
                ...
                fast_result = run_super_quick_field3(
                    ...
                    progress_callback=_super_quick_progress,
                )
                st.session_state["settings_run_status_20260617"] = fast_result
                st.success(
                    f"Super Quick completed instantly from loaded cache for {len(fast_symbols)} symbol(s)."
                    + (" Cached result reused." if fast_result.get("reused_cached_calculation") else "")
                )
                _safe_rerun()
                return
            except Exception as super_fast_exc:
                # Super Quick is deliberately compute-only.  Falling through
                # to the generic instant-run queue after a fast-lane error
                # leaves the old protected job in RUNNING state and can keep
                # Field 3 locked for a very long time.  Fail this click
                # explicitly, release every cooperative lock, and preserve the
                # last valid Field 3 result so the user can retry immediately.
                ...
                st.session_state["settings_run_status_20260617"] = {
                    "status": "FAILED",
                    "ok": False,
                    "calculation_scope": "SUPER_QUICK_FIELD3_THREE_TABLES",
                    "error": error_text,
                    "selected_symbols": list(fast_symbols),
                    "completed_symbols": 0,
                    "failed_symbols": len(fast_symbols),
                }
                for _lock_key in (
                    "instant_run_engine_running_20260705",
                    "settings_one_click_running_20260624",
                    "multi_symbol_run_in_progress_20260701",
                ):
                    st.session_state[_lock_key] = False
                st.error(
                    "Super Quick stopped safely before the slow calculation queue was started. "
                    f"{error_text}"
                )
                return
    ...
    def _settings_run_action_20260624():
        ...
        with st.spinner(spinner_text_20260707):
            if isinstance(loaded_run_marker_20260707, Mapping):
                ...
                refresh_result = {
                    ...
                    "timeframe": st.session_state.get("timeframe") or "H1",
                    "provider_requests": 0,
        ...
        if field10_fast_lane_20260709:
            calculation_status["one_hour_direction_confirmation_20260626"] = defer_to_quick(
                ...
                reason="Super Quick opens Field 3 as soon as its three compact requested sections are ready; all heavier calculations are completed by Quick/Full.",
            )
        else:
            try:
        ...
        with st.container(border=True):
            ...
            if job_status == "QUEUED" and not is_due(instant_job):
                ...
                def _execute_existing_protected_transaction(job: Mapping[str, Any], publish_callback):
                    ...
                    _live_render_gate_20260727 = {"percent": -10.0}
                    def _publish_and_render_live(snapshot: Mapping[str, Any]) -> None:
                        nonlocal instant_job
                        publish_callback(snapshot)
                        incoming_percent = float(snapshot.get("overall_percent") or snapshot.get("progress_percent") or 0.0)
                        should_render = (
                            incoming_percent >= 99.9
                            or incoming_percent - float(_live_render_gate_20260727["percent"]) >= 8.0
                        )
                        if not should_render:
                            return
                        _live_render_gate_20260727["percent"] = incoming_percent
                        try:

...
def render(vm):
    ...
    st.markdown("### Field 456 — Combined Decision and Preparation Evidence")
    ...
    tabs=st.tabs(["Field 4 Existing Output","Field 6 Existing Output","Field 1 Synchronization Summary"])
    with tabs[0]:
        from lunch.field_04.renderer import render as r; r(vm)
    with tabs[1]:
        from lunch.field_06.renderer import render as r; r(vm)
    with tabs[2]: st.json({"decision":s.decision,"run_id":s.run_id,"generation_id":s.generation_id,"snapshot_hash":s.source_snapshot_hash})

...
Main features:
    ...
    - Strategy Decision: strongest two S1-S6 scores / None
...
from core.fixed_fx_universe_20260918 import TARGET_FX_SET, filter_target_symbols
...
# Individual pullback model visibility columns.
# These expose the internal S1/S2/S3 checks so CSV exports can be audited.
STATISTICAL_PULLBACK_COLUMN = "Statistical Pullback"
VOLATILITY_PULLBACK_COLUMN = "Volatility Pullback"
EXHAUSTION_PULLBACK_COLUMN = "Exhaustion Pullback"
# Static ranked pullback columns (S1-S6). These are scores, not WAIT/ENTRY states.
STATIC_PULLBACK_COLUMNS = {f"S{i} Static Pullback": f"S{i}" for i in range(1, 7)}
...
def _ensure_required_audit_columns(frame: pd.DataFrame) -> pd.DataFrame:
    """Keep the eight audit columns present in every UI/export dataframe."""
    if not isinstance(frame, pd.DataFrame):
        return pd.DataFrame()
    out = frame.copy()
    aliases = {
    ...
    # Execution-model display columns use an explicit terminal state instead of
    # WAIT.  A non-triggered strategy is still a completed calculation.
    for col in (STATISTICAL_PULLBACK_COLUMN, VOLATILITY_PULLBACK_COLUMN, EXHAUSTION_PULLBACK_COLUMN):
        if col not in out.columns:
            out[col] = "NO ENTRY"
        else:
            out[col] = out[col].replace({"WAIT": "NO ENTRY", "": "NO ENTRY"}).fillna("NO ENTRY")

    # S1-S6 are continuous ranking scores.  They must always be numeric and
    # must never be repopulated with a WAIT placeholder after table reordering.
    for col in STATIC_PULLBACK_COLUMNS:
        if col not in out.columns:
            out[col] = 0
        out[col] = pd.to_numeric(out[col], errors="coerce").fillna(0).clip(0, 100).round().astype(int)
    if STRATEGY_DECISION_COLUMN not in out.columns:
...
# Pullback signal frequency control.
# Prevent one model from flooding the decision table.
PULLBACK_MIN_HOURLY_CONFIRMATIONS = 2
...
def _apply_entry_confirmation_rules(frame: pd.DataFrame) -> pd.DataFrame:
    """Keep each pullback strategy independent.

    Entry conditions belong to each S1-S6 model itself. Strategy Decision is
    only a display layer that lists triggered strategies and must not add a
    cross-strategy confirmation counter or ranking filter.
    """
    return frame


def _strategy_decision_from_scores(scores: Mapping[str, Any], bias: str, threshold: int = 70) -> str:
    """Return every S1-S6 strategy that meets the display entry threshold.

    Middle Standard Ranking and Finder CSV must use the same decision logic.
    Do not reduce the result to top-2 only because S5/S6 would disappear.
    """
    if bias not in {"BUY", "SELL"}:
        return NONE_LABEL
    decisions = []
    for label in ("S1", "S2", "S3", "S4", "S5", "S6"):
        value = pd.to_numeric(pd.Series([scores.get(label, 0)]), errors="coerce").fillna(0).iloc[0]
        if float(value) >= threshold:
            decisions.append(f"{bias} — {label}")
    return " | ".join(decisions) if decisions else NONE_LABEL


def _top_ranked_strategy_decision(scores: Mapping[str, Any], bias: str, *, top_n: int = 2) -> str:
    """Backward-compatible wrapper now returning all triggered strategies."""
    return _strategy_decision_from_scores(scores, bias)


def get_strategy_decision(
    row: pd.Series,
) -> str:
    """Display all strategies whose own entry conditions are triggered.

    This is not a ranking engine. It does not select a winner. S1-S4 remain
    high-standard pullback models and S5-S6 represent middle-standard bias
    pullback opportunities.
    """
    columns = _resolve_strategy_source_columns(row.to_frame().T)
    hrb = _row_bias(row, columns["HRB"])
    if hrb not in {"BUY", "SELL"}:
        return NONE_LABEL
    ...
    # Strategy Decision is only a display layer.
    # It lists strategies whose own static pullback score reaches the entry
    # threshold. It does not rank strategies against each other.
    scores = ranked_pullback_scores(history, hrb)
    for label in ("S1", "S2", "S3", "S4", "S5", "S6"):
        if scores.get(label, 0) >= 70:
            decisions.append(f"{hrb} — {label}")

    return " | ".join(dict.fromkeys(decisions)) if decisions else NONE_LABEL
...
def _add_strategy_decision_column(
    ...
    # Previous logic reused the complete multi-symbol table for every row,
    # which made one symbol's OHLC/pullback values leak into other symbols.
    ...
    # Expose individual S1/S2/S3 model states. Previously these were only
    # evaluated internally and the CSV only showed the final Strategy Decision.
    # These columns make it possible to verify that the new pullback engines
    # are actually running.
    def _model_state(row, fn, label):
        bias = _row_bias(row, _resolve_strategy_source_columns(row.to_frame().T)["HRB"])
        history = row.get("_history_frame")
        if bias not in {"BUY", "SELL"} or not isinstance(history, pd.DataFrame):
            return "NO ENTRY"
        value = fn(history, bias)
        return "NO ENTRY" if str(value).strip().upper() == "WAIT" else value

    result[STATISTICAL_PULLBACK_COLUMN] = result.apply(
        lambda r: _model_state(r, statistical_pullback, S1_LABEL), axis=1
    )
    result[VOLATILITY_PULLBACK_COLUMN] = result.apply(
        lambda r: _model_state(r, volatility_pullback, S2_LABEL), axis=1
    )
    result[EXHAUSTION_PULLBACK_COLUMN] = result.apply(
        lambda r: _model_state(r, exhaustion_pullback, S3_LABEL), axis=1
    )

    # Always publish static ranked pullback strength for S1-S6.
    def _static_rank(row):
        bias = _row_bias(row, _resolve_strategy_source_columns(row.to_frame().T)["HRB"])
        history = row.get("_history_frame")
        if bias not in {"BUY", "SELL"} or not isinstance(history, pd.DataFrame):
            return {f"S{i} Static Pullback": 0 for i in range(1, 7)}
        scores = ranked_pullback_scores(history, bias)
        return {f"S{i} Static Pullback": scores.get(f"S{i}", 0) for i in range(1, 7)}

    static_scores = result.apply(_static_rank, axis=1)
    for col in STATIC_PULLBACK_COLUMNS:
        result[col] = static_scores.apply(lambda x, c=col: x[c])
    ...
    result = _apply_entry_confirmation_rules(result)
...
def _attach_live_pullback_data(frame: pd.DataFrame, state: Mapping[str, Any] | None = None) -> pd.DataFrame:
    """Attach current per-symbol OHLC plus fully recalculated S1-S6 results.

    Every loaded symbol is evaluated from its own canonical candle frame.  The
    six static pullback fields are numeric 0-100 scores and are recalculated on
    every render/export; they are never left as WAIT placeholders.
    """
    ...
    if isinstance(state, Mapping):
        for key in ("canonical_symbol_candles", "symbol_history", "symbol_histories", "ohlc_by_symbol", "multi_symbol_candles", "loaded_symbol_frames"):
            value = state.get(key)
            if isinstance(value, Mapping):
                for symbol, candidate in value.items():
                    if isinstance(candidate, pd.DataFrame) and not candidate.empty:
                        normalized = str(symbol).strip().upper().replace("/", "")
                        if normalized in TARGET_FX_SET:
                            symbol_histories[normalized] = candidate.copy()
                if symbol_histories:
                    break

    # Legacy single-frame fallback is valid only for its own symbol.
    if not symbol_histories and isinstance(state, Mapping):
        active_symbol = str(state.get("symbol") or state.get("requested_symbol_20260629") or "").strip().upper().replace("/", "")
        for key in ("last_df", "shared_df", "market_df", "ohlc_df"):
            candidate = state.get(key)
            if active_symbol in TARGET_FX_SET and isinstance(candidate, pd.DataFrame) and not candidate.empty:
                symbol_histories[active_symbol] = candidate.copy()
                break
    ...
    for symbol, hist in symbol_histories.items():
        if not all(c in hist.columns for c in ("open", "high", "low", "close")):
            continue
        h = hist.copy()
    ...
    strategy_decisions: dict[Any, str] = {}
    for idx, row in out.iterrows():
        symbol = str(row.get("Symbol", "")).strip().upper().replace("/", "")
        hist = prepared.get(symbol)
        if hist is None or hist.empty:
            # Keep a completed, explicit result instead of WAIT.  Loaded rows
            # should normally never reach this branch because the canonical
            # loader now feeds Super Quick all merged selector frames.
            for col in STATIC_PULLBACK_COLUMNS:
                out.at[idx, col] = 0
            out.at[idx, STATISTICAL_PULLBACK_COLUMN] = "NO DATA"
            out.at[idx, VOLATILITY_PULLBACK_COLUMN] = "NO DATA"
            out.at[idx, EXHAUSTION_PULLBACK_COLUMN] = "NO DATA"
            strategy_decisions[idx] = NONE_LABEL
            continue
        latest = hist.iloc[-1]
        for label, source in (("Open Price", "open"), ("Close Price", "close"), ("Highest Price", "high"), ("Lowest Price", "low")):
            out.at[idx, label] = float(latest[source])
        bias = _row_bias(row, _resolve_strategy_source_columns(out.loc[[idx]])["HRB"])
        score_bias = bias if bias in {"BUY", "SELL"} else "NEUTRAL"
        scores = ranked_pullback_scores(hist, score_bias)
        for col, score_key in STATIC_PULLBACK_COLUMNS.items():
            out.at[idx, col] = int(scores.get(score_key, 0) or 0)

        if bias in {"BUY", "SELL"}:
            s1_state = statistical_pullback(hist, bias)
            s2_state = volatility_pullback(hist, bias)
            s3_state = exhaustion_pullback(hist, bias)
            out.at[idx, STATISTICAL_PULLBACK_COLUMN] = "NO ENTRY" if s1_state == "WAIT" else s1_state
            out.at[idx, VOLATILITY_PULLBACK_COLUMN] = "NO ENTRY" if s2_state == "WAIT" else s2_state
            out.at[idx, EXHAUSTION_PULLBACK_COLUMN] = "NO ENTRY" if s3_state == "WAIT" else s3_state
        else:
            out.at[idx, STATISTICAL_PULLBACK_COLUMN] = "NO ENTRY"
            out.at[idx, VOLATILITY_PULLBACK_COLUMN] = "NO ENTRY"
            out.at[idx, EXHAUSTION_PULLBACK_COLUMN] = "NO ENTRY"

        strategy_decisions[idx] = _top_ranked_strategy_decision(scores, bias, top_n=2)

    out[STRATEGY_DECISION_COLUMN] = [strategy_decisions.get(idx, NONE_LABEL) for idx in out.index]
    out = _ensure_required_audit_columns(out)
    out = _apply_entry_confirmation_rules(out)
    if "Avg Regime Candle Rank" in out.columns:
        cols = [c for c in out.columns if c != STRATEGY_DECISION_COLUMN]
        cols.insert(cols.index("Avg Regime Candle Rank"), STRATEGY_DECISION_COLUMN)
        out = out[cols]
    return out
...
def _age_only_ranking(
    ...
    ordered = [
        ...
        SL_DECISION_COLUMN,
        "Symbol",
        STATISTICAL_PULLBACK_COLUMN,
        VOLATILITY_PULLBACK_COLUMN,
        EXHAUSTION_PULLBACK_COLUMN,
        STRATEGY_DECISION_COLUMN,
        ...
        render_field3_table(
            ...
            column_config={
                STRATEGY_DECISION_COLUMN: st.column_config.TextColumn(
                    STRATEGY_DECISION_COLUMN,
                    width="large",
                    help="Direction first, then the two strongest recalculated S1-S6 strategy scores.",
                ),
            },
        )
...
def _load_middle_regime_history(state: MutableMapping[str, Any]) -> pd.DataFrame:
    """Load Finder history quickly without replaying pullback models row-by-row."""
    history = state.get(MIDDLE_HISTORY_STATE_KEY)
    if not isinstance(history, pd.DataFrame) or history.empty:
        history = pd.DataFrame()
        try:
            if MIDDLE_HISTORY_FILE.exists():
                history = pd.read_parquet(MIDDLE_HISTORY_FILE)
        except Exception as exc:
            state["field3_middle_history_load_error_20260722"] = f"{type(exc).__name__}: {exc}"
        if history.empty:
            try:
                if MIDDLE_HISTORY_CSV_FILE.exists():
                    history = pd.read_csv(MIDDLE_HISTORY_CSV_FILE, low_memory=False)
            except Exception as exc:
    ...
    if "Datetime" not in history.columns:
        fallback = _find_datetime_column(history)
        if fallback is None:
            return pd.DataFrame()
        history = history.rename(columns={fallback: "Datetime"})
    history["Datetime"] = _normalize_datetime_series(history["Datetime"])
    history = history.dropna(subset=["Datetime"])
    if "Symbol" in history.columns:
        normalized = history["Symbol"].astype(str).str.upper().str.replace("/", "", regex=False).str.replace("_", "", regex=False).str.replace(" ", "", regex=False)
        history = history.loc[normalized.isin(TARGET_FX_SET)].copy()
        history["Symbol"] = normalized.loc[history.index]
    subset = ["Datetime", "Symbol"] if "Symbol" in history.columns else ["Datetime"]
    history = history.drop_duplicates(subset=subset, keep="last").sort_values(subset, kind="mergesort").reset_index(drop=True)
    history = _add_ras_decision_column(history)
    # Historical Finder intentionally does not replay S1/S2/S3 HMMs for every
    # candle.  Preserve already-generated decisions; otherwise expose the cheap
    # S4 rule and apply the same two-confirmations-per-hour condition.
    # Rebuild Finder Strategy Decision from the same S1-S6 score columns used by
    # the live Middle Standard Ranking table. Older snapshots only contained an
    # S4 fallback, which hid S1/S2/S3/S5/S6 from CSV exports.
    higher_col = next((c for c in ("Higher Standard Regime Bias", "Higher Regime Bias", "Higher Bias") if c in history.columns), None)
    if higher_col is not None:
        def _finder_decision(row):
            bias = _normalize_bias(row.get(higher_col))
            scores = {
                f"S{i}": pd.to_numeric(row.get(f"S{i} Static Pullback", row.get(f"S{i}" , 0)), errors="coerce") if pd.notna(row.get(f"S{i} Static Pullback", row.get(f"S{i}", 0))) else 0
                for i in range(1, 7)
            }
            return _strategy_decision_from_scores(scores, bias)
        history[STRATEGY_DECISION_COLUMN] = history.apply(_finder_decision, axis=1)
    if STRATEGY_DECISION_COLUMN in history.columns:
        history = _apply_entry_confirmation_rules(history)
...
def _save_middle_regime_history_snapshot(
    ...
    snapshot = _sanitize_index_column_collision(middle_age).drop(
        columns=[SL_DECISION_COLUMN, WEIGHTED_RISK_DECISION_COLUMN, WEIGHTED_RISK_SCORE_COLUMN], errors="ignore"
    )
...
def _run_finder_history_build_on_demand(state: MutableMapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        # Finder is a historical export, so it must use the complete validated
        # multi-symbol registry rather than only the active/completed symbol
        # list.  The latter can temporarily contain only a partial subset after a
        # reconnect or a partial API run even though other symbol frames are
        # already present in MARKET_RESULTS_KEY.
        ...
        discovered = [
            ...
            for symbol in raw_results.keys()
            if str(symbol).strip()
        ]
        configured = [
            str(symbol).strip().upper()
            for symbol in (
                list(context.loaded_symbols)
                + list(context.completed_symbols)
                + list(context.configured_symbols)
            )
            if str(symbol).strip()
        ]
        symbols = filter_target_symbols(discovered + configured, limit=20)
        if not symbols:
            symbols = filter_target_symbols(list(TARGET_FX_SET), limit=20)
        frames, failures = _extract_loaded_frames(state, symbols)
        if not frames:
            # A single-symbol load is kept in last_df by the fast path even
            # when the multi-symbol registry is not populated.  Use it so
            # Finder can still build history instead of reporting the opaque
            # NO_LOADED_FRAMES error.
            fallback = state.get("last_df")
            if isinstance(fallback, pd.DataFrame) and not fallback.empty:
                required = {"open", "high", "low", "close"}
                if required.issubset(fallback.columns):
                    symbol = (symbols[0] if symbols else "AUDCAD")
                    fallback = fallback.copy()
                    if "open_time" not in fallback.columns:
                        for candidate in ("Datetime", "datetime", "time", "timestamp"):
                            if candidate in fallback.columns:
                                fallback["open_time"] = _normalize_datetime_series(fallback[candidate])
                                break
                    if "open_time" in fallback.columns:
                        frames = {symbol: fallback.dropna(subset=["open_time"]).reset_index(drop=True)}
            if not frames:
                return {"ok": False, "status": "NO_LOADED_FRAMES", "rows": 0}
        # Do not truncate Finder frames to a shared cutoff.  The Finder must
        # export the full available history for each symbol (a union of
        # per-symbol ranges); shared-cutoff alignment is only appropriate for
        # the live cross-symbol ranking calculation.
        aligned = {
            symbol: frame.copy().reset_index(drop=True)
